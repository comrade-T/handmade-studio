...
const Node = union(enum) {
    ...
    /// Create Leaves from given `source`. Each Leaf.buf's length restricted to `capacity_per_leaf`.
    ///
    /// Currently unused, due to author trying to experiment not splitting the source at all.
    /// Instead in favor of creating 1 single Leaf from the source.
    fn createLeavesByCapacity(a: Allocator, source: []const u8, capacity_per_leaf: usize) ![]Node {
